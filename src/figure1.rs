...
//!
//! This is the reference program the whole POC is aimed at, so it lives in
//! the library rather than in the example that prints it: the analysis tests
//! in `tests/` and the `figure1` example both build on it.
//!
//! ```java
//!  1 public class Obj{}
...
use crate::access_path::{AccessPath, Constraint, Summary};
use crate::ir::*;
...
pub fn figure2_summaries() -> BTreeMap<Proc, Summary> {
    ...
    summaries.insert(
        p("FacadeImpl.id"),
        Summary::from([Constraint::Path {
            sup: ret("FacadeImpl.id"),
            sub: par("FacadeImpl.id", 1),
        }]),
    );
            ...
            for constraint in &summary {
                for path in constraint.paths() {
                    match &path.base {
                        ...
                        // Figure 2 is the context-insensitive oracle: no
                        // locals, and no deferred critical statements.
                        other => unreachable!("{other} has no place in Figure 2"),
                    }
                }
